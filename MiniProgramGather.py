    ...
    def _formatResChannelData(self, reqs_json_dict: dict, field_list: list):
        ...
        if isinstance(sequence_data_list, list) and len(sequence_data_list) > 0:
            for i in range(0, len(sequence_data_list)):
                ...
                for item in point_list:
                    temp = {field_name: item.get('value', 0), 'day': item['label'], 'wxgamecid': wxgamecid,
            ...
            for i in range(0, len(group_data_list)):
                ...
                for item in point_list:
                    if int(item.get('label_value', 0)) == 33:  # 只取微信广告，其他没用
                        temp = {field_name: item.get('value', 0), 'day': mytools.unixTimeDate(int(day_uix)),
                                'group_value': 4, 'appId': self.app_info['appid']}
                        res.append(temp)
        return res
